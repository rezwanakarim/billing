    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="px-4 ">
            <ol class="flex text-sm justify-end text-gray-500">
                ...
                <li class="mr-3">/ </li>
                <li>
                    <a href="{{ route('foodItems.index') }}" class="text-gray-900 dark:text-white">
                        Food Item</a>
                </li>
            </ol>
        </div>
                    ...
                    <div class="p-8 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                        <table class="w-full">
                            ...
                            <tbody>
                                ...
                                @php
                                    $starting_serial = ($foodItems->currentPage() - 1) * $foodItems->perPage() + 1;
                                @endphp
                                @foreach ($foodItems as $key => $foodItem)
                                    <tr class="border-t border-gray-200 dark:border-gray-700">
                                        <td class="px-4 py-3">{{ $starting_serial++ }}</td>
                                        <td class="px-4 py-3">{{ $foodItem->name }}</td>
                                        <td class="px-4 py-3">
                                            <a href="{{ route('foodItems.show', $foodItem->id) }}"
                                            ...
                                            <a href="{{ route('foodItems.edit', $foodItem->id) }}"
                                            ...
                                            <form action="{{ route('foodItems.destroy', $foodItem->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    ...
                                                    onclick="return confirm('Are you sure you want to delete this foodItem?')">
                                                    Delete
                                                </button>
                                            </form>
                                        </td>
                    ...
                    <div class="flex items-center justify-start mt-4">
                        <nav class="flex items-center">
                            ...
                            @else
                                ...
                                <a href="{{ $foodItems->previousPageUrl() }}"
                            ...
                            <span class="px-2 py-1 text-sm font-medium leading-5 text-gray-600">
                                Page: {{ $foodItems->currentPage() }} of {{ $foodItems->lastPage() }} Records:
                                {{ $foodItems->total() }}
                            </span>

                            {{-- Next Page Link --}}
                            @if ($foodItems->hasMorePages())
                                <a href="{{ $foodItems->nextPageUrl() }}"
                                ...
                                <a href="{{ $foodItems->url($foodItems->lastPage()) }}"
